...
public class InitialData {
	...
	public InitialData() throws ParseException { // TODO these need to be run in
													// a specific order
		setOrderInitialData();
	...
	public ArrayList<Supplier> getSupplier() {
		...
		//
		ArrayList<Address> address = getAddresses();
	...
	public ArrayList<Payment> getPayment() throws ParseException {
		 ...
		 * instantiated. The method: - Creates an ArrayList of Payments,
		 * SimpleDateformat object and a User ArrayList object which is
		 * initialised from the getUser() method - adds each user to the payment
		 * ArrayList with an arbitrary card number, card type and date (which is
		 * parsed) and uses the users ArrayList to get the name and the instance
		 * of the object to the Payment ArrayList - returns the Payment
		 * ArrayList
		 */
		...
		payment.add(new Payment("1234567891011121", "debit", sdf.parse("06/25"), "Mr W Seaford",
				new Address("23 Holway Road", "TA1 2EZ"), users.get(0)));
		payment.add(new Payment("1234678901246789", "credit", sdf.parse("02/24"), "Mr S Smith",
				new Address("71 Mountbatton", "BS37 S35"), users.get(1)));
		payment.add(new Payment("0987654321234567", "debit", sdf.parse("04/17"), "Miss E Smitten",
				new Address("1 Beaumont Court", "E1 4NX"), users.get(2)));
		payment.add(new Payment("1357708642135790", "credit", sdf.parse("02/26"), "Mr W T Beaumont",
				new Address("14 Louise Road", "E15 4NW"), users.get(3)));
		payment.add(new Payment("2468097531246809", "debit", sdf.parse("02/19"), "Mrs D Matthison",
	...
	private void setOrderInitialData() throws ParseException {
		...
		for (int i = 0; i < 5; i++) {
			orders.add(new Order(price[i], new Date(), new Date(), getPayment().get(i), getAddresses().get(i),
					getUser().get(i), getOrderStatuses().get(i)));
		}
	}
	...
	public ArrayList<Credit> getCredit() {
		...
		/*
		 * Author : willseaford This method returns an ArrayList of Credit
		 * objects and has a dependency on the User object class being
		 * instantiated. The method: - Creates an ArrayList of Credit and a User
		 * ArrayList object which is initialised from the getUser() method -
		 * adds each user to the Credit with their amount balance and amount
		 * remaining with the quarter - returns the Credit ArrayList
		 */
	...
	public ArrayList<LineItems> getLineItems() throws ParseException {
		...
		/*
		 * Author : willseaford This method returns an ArrayList of LineItems
		 * objects and has a dependency on the order and product object classes
		 * being instantiated. The method: - Creates ArrayLists of LineItems,
		 * Order and Product respectively and initialises order and product with
		 * values from in-class getter method - adds each instance order and
		 * product to the Credit object with the quantity, subtotal and
		 * returnedQty and adds the Credit object to the ArrayList. - returns
		 * the LineItems ArrayList
		 */
	...
	public ArrayList<EmployeeUser> getEmployeeUser() {
		...
		/*
		 * Author : willseaford This method returns an ArrayList of EmployeeUser
		 * objects and has a dependency on the AccountType class being
		 * instantiated. The method: - Creates ArrayLists of EmployeeUser and
		 * AccountType and initialises AccountType - adds each instance. -
	...
	private void setSupplierOrderInitialdata() {
		...
		for (int i = 0; i < 5; i++) {
			supplierOrders.add(new SupplierOrder(getProducts().get(i), getOrderStatuses().get(i), getSupplier().get(i),
					i, new Date(), received[i]));
		}
	}
